...
--   2. Add public.org_id() helper (returns current user's org)
--   3. Add org_id column to every data table
--   4. BEFORE INSERT triggers auto-set org_id = public.org_id()
--   5. RLS policies updated to scope by org_id
--   6. Migrate existing data: one org per existing user
-- ============================================================
...
-- ============================================================
-- ORGANIZATION_MEMBERS (must exist before organizations policies)
-- ============================================================
...
-- ============================================================
-- HELPER: public.org_id() — returns current user's org
-- Used in RLS policies and trigger defaults below.
-- ============================================================
...
-- Create one org per existing user, assign all their data to it.
-- ============================================================
DO $$
DECLARE
  r RECORD;
...
-- ============================================================
-- AUTO-SET org_id ON INSERT via triggers
-- ============================================================
...
-- ============================================================
-- UPDATE RLS POLICIES — scope everything by org_id
-- ============================================================
...
-- contact_tags (scoped via contacts join)
...
CREATE POLICY "Org members can manage contact tags" ON contact_tags
  FOR ALL USING (
    EXISTS (SELECT 1 FROM contacts WHERE contacts.id = contact_tags.contact_id AND contacts.org_id = public.org_id())
  );
...
CREATE POLICY "Org members can manage custom fields" ON custom_fields
  FOR ALL USING (org_id = public.org_id());
...
CREATE POLICY "Org members can manage custom values" ON contact_custom_values
  FOR ALL USING (
    EXISTS (SELECT 1 FROM contacts WHERE contacts.id = contact_custom_values.contact_id AND contacts.org_id = public.org_id())
  );
...
CREATE POLICY "Org members can manage notes" ON contact_notes
  FOR ALL USING (
    EXISTS (SELECT 1 FROM contacts WHERE contacts.id = contact_notes.contact_id AND contacts.org_id = public.org_id())
  );

-- conversations
DROP POLICY IF EXISTS "Users can manage own conversations" ON conversations;
DROP POLICY IF EXISTS "Org members can manage conversations" ON conversations;
CREATE POLICY "Org members can manage conversations" ON conversations
  FOR ALL USING (org_id = public.org_id());
...
CREATE POLICY "Org members can view messages" ON messages
  FOR ALL USING (
    EXISTS (SELECT 1 FROM conversations WHERE conversations.id = messages.conversation_id AND conversations.org_id = public.org_id())
  );
...
CREATE POLICY "Org members can manage whatsapp config" ON whatsapp_config
  FOR ALL USING (org_id = public.org_id());
...
CREATE POLICY "Org members can manage templates" ON message_templates
  FOR ALL USING (org_id = public.org_id());
...
CREATE POLICY "Org members can manage pipelines" ON pipelines
  FOR ALL USING (org_id = public.org_id());
...
CREATE POLICY "Org members can manage pipeline stages" ON pipeline_stages
  FOR ALL USING (
    EXISTS (SELECT 1 FROM pipelines WHERE pipelines.id = pipeline_stages.pipeline_id AND pipelines.org_id = public.org_id())
  );
...
CREATE POLICY "Org members can manage broadcasts" ON broadcasts
  FOR ALL USING (org_id = public.org_id());
...
CREATE POLICY "Org members can manage broadcast recipients" ON broadcast_recipients
  FOR ALL USING (
    EXISTS (SELECT 1 FROM broadcasts WHERE broadcasts.id = broadcast_recipients.broadcast_id AND broadcasts.org_id = public.org_id())
  );
...
CREATE POLICY "Org members can manage automations" ON automations
  FOR ALL USING (org_id = public.org_id());

-- automation_steps (scoped via automations join)
...
CREATE POLICY "Org members can manage automation steps" ON automation_steps
  FOR ALL USING (
    EXISTS (SELECT 1 FROM automations WHERE automations.id = automation_steps.automation_id AND automations.org_id = public.org_id())
  );
...
CREATE POLICY "Org members can view automation logs" ON automation_logs
  FOR ALL USING (
    EXISTS (SELECT 1 FROM automations WHERE automations.id = automation_logs.automation_id AND automations.org_id = public.org_id())
  );

-- ============================================================
-- UPDATE handle_new_user: auto-create org for new signups
-- ============================================================
